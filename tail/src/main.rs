/*
 написати власну імплементацію лінуксової tail -n {count} {file} - вивести останні N стрічок з файлу.

 важливо:
 має працювати швидко навіть з гіганськими файлами (більше гігабайта)
 працюємо з байтами напряму (ігноруємо utf8), межа стрічки визначається b'\n' символом
 для парсення аргументів треба використати бібліотеку clap (https://docs.rs/clap/4.5.49/clap/)

 якщо занадто швидко впораєтеся і хочете трошки челенджу:
 додаємо підтримку utf8, межа стрічки визначається стандартним "\n" чаром


швидкість виконання своєї програми можете перевірити за допомогою команди time:
time tail -n 100 system.log
>> ...
>> tail -n 100 system.log  0,00s user 0,00s system 26% cpu 0,012 total

тільки не забудьте збілдати релізну версію для бенчмарка:
cargo build --release
 */
